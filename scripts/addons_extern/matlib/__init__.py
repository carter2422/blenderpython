...
def sendmat(name):
	...
	if add:
		...
		#print(scriptpath)
		#print("sendmat: No Existe mando material "+name)
		#print("this", thispath)
...
#		   elem = wml.getElementsByTagName(matname)
#		   #print(elem)
#		   for el in elem:
#			   deleteNode(el)
#		   elem = wml.getElementsByTagName(catname)
#		   
##	 print("----------")
##   
#		   if len(elem)>0:
#			   elem=elem[0]
#			   node = doc.createElement(matname)
...
class matlibvxOperator(bpy.types.Operator):
	...
	def invoke(self, context, event):
		...
		if add=="ADD":
			...
			if len(matlib)>0 and index>-1:
				...
				if dopreview:
					...
					if scn.objects.active!=obj:
						matlibPropGroup.lastselected = scn.objects.active 
			...
			if scn.objects.active==obj:
				scn.objects.active = matlibPropGroup.lastselected
