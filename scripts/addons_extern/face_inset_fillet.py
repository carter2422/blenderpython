...
class fif_op0(bpy.types.Operator):
    ...
    en0 = EnumProperty( items =( ('opt0', '1', ''), ('opt1', '2', '') ), name = '', default = 'opt0' )
    ...
    def draw(self, context):
        ...
        row1 = box.split(0.40, align = True)
        row1.label('Inset amount:')
        row1.prop(self, 'opp')

        row2 = box.split(0.60, align = True)
        row2.label('Number of sides:')
        row2.prop(self, 'n_', slider = True)

        box.prop(self, 'radius')

        row3 = box.split(0.40, align = True)
        if self.radius == True:
            row3.label('Radius:')
        else:
            row3.label('Distance:')
