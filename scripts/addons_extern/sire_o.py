...
# ------ ------ rotate
def rotate_(bme, list_0, pivot, ax, b, ang, no):
...
class so_p0(bpy.types.Panel):
    ...
    def draw(self, context):
        ...
        if en == 'opt0':
            ...
            row1 = layout.split(0.40, align = True)
            row1.operator('so.op8_id', text = 'Apply')
...
class so_op0(bpy.types.Operator):
    ...
    def execute(self, context):
        ...
        collection_.add()
        so_buf.i += 1
        collection_[-1].name = so_buf.name      # str(so_buf.i) + ' ' + so_buf.name
...
class so_op6(bpy.types.Operator):
    ...
    def execute(self, context):
        ...
        if len(list_0) == 0:
            ...
            if ax == 'ar':
                if len(so_buf.list_axis) != 2:
                    self.report({'INFO'}, 'No axis  stored in memory unable to continue.')
                    edit_mode_in()
                    return {'CANCELLED'}
            elif ax == 'nn':
                    ...
                    if len(list_1) < 2:
                        self.report({'INFO'}, 'At least two edges needed to calculate normal unable to continue.')
                        edit_mode_in()
                        return {'CANCELLED'}
                    else:
                        no = get_no_(bme, dict_0)
                        if no == None:
                            self.report({'INFO'}, 'Angle between all edges is 0 or 180 can not calculate normal unable to continue.')
                            edit_mode_in()
                            return {'CANCELLED'}

            if pivot == 'par':
                if len(so_buf.list_pivot) == 0:
                    self.report({'INFO'}, 'No pivot point stored in memory unable to continue.')
                    edit_mode_in()
                    return {'CANCELLED'}

            rotate_(bme, list_0, pivot, ax, b, ang, no)
...
class so_op8(bpy.types.Operator):
    ...
    def execute(self, context):
        ...
        if len(context.scene.so_custom_collection_) == 0:
            self.report({'INFO'}, 'No actions available unable to continue.')
            edit_mode_in()
            return {'CANCELLED'}
        else:
