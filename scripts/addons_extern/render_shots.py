...
def init_props(scn):
    ...
    object.rs_shot_start = IntProperty(name="Start",
        ...
        default=scn.frame_start, min=0, max=300000)
    
    object.rs_shot_end = IntProperty(name="End",
        description="Last frame in this shot", 
        default=scn.frame_end, min=0, max=300000)
    ...
    # Other
    scene.rs_is_updating = BoolProperty()
    
    scene.rs_create_folders = BoolProperty()
...
class RENDER_OT_RenderShots_settingshelp(bpy.types.Operator):
    ...
    cam = StringProperty()
    _cycles = using_cycles(bpy.context.scene)
    ...
    def execute(self, context):
        ...
        scn = context.scene
        render_settings_handler(scn, 'GET', self._cycles, None)
    ...
    def modal(self, context, event):
        ...
        if event.type in {'ESC'}:
            global RENDER_SETTINGS_HELP
            RENDER_SETTINGS_HELP = False
            render_settings_handler(scn, 'REVERT', self._cycles, None)
            return {'FINISHED'}
    ...
    def modal(self, context, event):
        ...
        if RENDER_DONE and self.camList:
            ...
            if ob["rs_settings_use"]:
                render_settings_handler(scn, 'SET', self._cycles, ob)
            else:
                render_settings_handler(scn, 'REVERT', self._cycles, None)
