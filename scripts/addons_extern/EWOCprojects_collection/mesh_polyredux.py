...
def my_mesh_util():
	...
	for vert in grid_data_dict.keys():
		for f in vert.link_faces:
			for idx in range(len(bm.loops.layers.uv)):
				for l in f.loops:
					uvdict[(idx, l.vert)] = l[bm.loops.layers.uv[idx]]
		face = bmesh.utils.face_join(vert.link_faces[:])
	for l in face.loops:
		for idx in range(len(bm.loops.layers.uv)):
			l[bm.loops.layers.uv[idx]] = uvdict[(idx, l.vert)]
