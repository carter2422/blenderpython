...
class VIEW3D_MT_edit_mesh_paleajed(bpy.types.Menu):
	...
	def draw(self, context):
		...
		layout.operator("mesh.deathguppie",
			text="DeathGuppie")
		layout.operator("object.creaprim",
			text="CreaPrim")
...
class PaleajedPanel(bpy.types.Panel):
	...
	def draw(self, context):
		...
		layout.operator("object.creaprim", text="CreaPrim")
		layout.prop(scn, "Name")
		layout.prop(scn, "Apply")
		...
		mesh_selproject.redomenus = 1	
		if not(mesh_selproject.started):
			mesh_selproject._handle3 = context.region.callback_add(mesh_selproject.setparam, (), 'POST_PIXEL')
